...
async function generateSounds(): Promise<SoundSeed[]> {
    ...
    for (const { id, title, artist } of EXISTING_SOUNDS) {
        ...
        try {
            ...
            sounds.push({ id, title, artist, peaks });
        } catch (err) {
            console.error(`Failed to generate peaks for sound ${id}`, err);
            // Dummy flat peaks if file is missing
            sounds.push({ id, title, artist, peaks: new Array(100).fill(0) });
        }
    }
    return sounds;
}
...
function generateConversationPairs(
    ...
    for (const userA of sortedUsers) {
        ...
        for (const userB of selected) {
            ...
            userPairCounts.set(
                userA.id,
                (userPairCounts.get(userA.id) || 0) + 1,
            );
            userPairCounts.set(
                userB.id,
                (userPairCounts.get(userB.id) || 0) + 1,
            );
        }
    }
    ...
    return weights.map((weight, index) => {
        ...
        const boundary =
            index === weights.length - 1
                ? extraDurationMs
                : Math.round(
                      (extraDurationMs * accumulatedWeight) / totalWeight,
                  );
...
function generateConversationTimestamps(messageCount: number): Date[] {
    ...
    const minimumConversationDurationMs =
        (messageCount - 1) * DM_MIN_MESSAGE_GAP_MS;
...
function determineLastReadAtForUser(
    ...
    const sentMessages = sortedConversation.filter(
            ...
            : ([...receivedMessages]
                  ...
                  .find(
                      (message) =>
                          ...
                          messageCreatedAtMs(latestSentMessage),
                  ) ?? null);
...
function generateDirectMessages(
    ...
    for (let i = 0; i < pairs.length; i++) {
        ...
        const lastReadAtForA = determineLastReadAtForUser(
            conversation,
            userA.id,
        );
        const lastReadAtForB = determineLastReadAtForUser(
            conversation,
            userB.id,
        );
